...
# Fonction pour tester des configurations spécifiques
def test_specific_configurations():
    base_data = '1ea5f8e0177447648e704996d715bcd9' # CHANGER ICI LE HASH
    base_data_bytes = bytes.fromhex(base_data)
    base_instance = Bob(base_data_bytes)
    base_hash = base_instance.hexdigest()

    print("Base hash:", base_hash)

    # Tester différentes longueurs en ajoutant des zéros devant
    for j in range(1250,4000):  # Test various additional zero lengths
        padding = bytes(j)  # Create a padding of zero bytes of length j
        test_data = padding + base_data_bytes  # Prepend zeros to the base data
        test_instance = Bob(test_data)
        test_hash = test_instance.hexdigest()
        
        #print(f"Test data (hex): {test_data.hex()}")  # Debugging
        if test_hash == base_hash:
            print(f"Collision found with extra length {j}")
            print(f"Base data (hex): {base_data}")
            print(f"Test data (hex): {test_data.hex()}")
            print(f"Base hash: {base_hash}")
            print(f"Test hash: {test_hash}")
            return

    print("No collision found with additional zeros.")
